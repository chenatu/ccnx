 ...
 **/
 public class ccnputpacket extends CommonOutput implements Usage {
	 static ccnputpacket ccnputpacket = new ccnputpacket();
	...
	public void write(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (CommonArguments.parseArguments(args, i, ccnputpacket)) {
		...
		try {
			...
			if (args.length == (CommonParameters.startArg + 3)) {
				int packetLength = Integer.parseInt(args[CommonParameters.startArg + 1]);
				int packetCount = Integer.parseInt(args[CommonParameters.startArg + 2]);
				doPut(handle, packetLength, packetCount, argName);
				if (CommonParameters.verbose)
					System.out.println("ccnputpacket took: "+(System.currentTimeMillis() - starttime)+" ms");
				System.exit(0);
			} else {
				usage(CommonArguments.getExtraUsage());
			}
	...
	public void usage(String extraUsage) {
		System.out.println("usage: ccnputpacket " + extraUsage + "[-v (verbose)] [-raw] [-unversioned] [-local | -allownonlocal] [-timeout millis] [-log level] [-as pathToKeystore] [-ac (access control)] <ccnname> (<filename>|<url>)*");
		System.exit(1);
	}

	public static void main(String[] args) {
		ccnputpacket.write(args);
	}
}
